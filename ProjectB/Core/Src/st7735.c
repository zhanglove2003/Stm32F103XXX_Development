...
void ST7735_SetRotation(uint8_t rotation)
{
  ...
  {
    case 0:
      madctl = ST7735_MADCTL_MV | ST7735_MADCTL_MODE;
      break;
    case 1:
...
void ST7735_ShowChinese(uint16_t x, uint16_t y, const uint8_t *chinese, uint16_t color, uint16_t bgColor)
{
  uint32_t i, j;

  ST7735_SetAddressWindow(x, y, x + 15, y + 15);
  ST7735_WriteCommand(0x2C);

  for (i = 0; i < 16; i++) {
    uint16_t line = (uint16_t)(chinese[(15 - i) * 2] << 8) | chinese[(15 - i) * 2 + 1];
    for (j = 0; j < 16; j++) {
      if (line & (0x8000 >> j)) {
        uint8_t data[] = { color >> 8, color & 0xFF };
        ST7735_WriteData(data, sizeof(data));
